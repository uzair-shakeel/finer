  ...
  return (
    <div className="max-w-[1296px] mx-auto w-full mt-[110px]">
      ...
      {allSelectedOptions.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-3 items-center justify-center bg-gray-100 p-4 rounded-lg max-w-[876px] mx-auto">
          {allSelectedOptions.map((option, index) => (
            <div
              key={index}
              className="flex items-center bg-white rounded-full px-4 py-1 text-sm font-medium text-gray-700"
            >
              <span>{option.label.toUpperCase()}</span>
              <button
                onClick={() => handleRemoveOption(option.filterType, option)}
          ...
          <button
            ...
            className="flex items-center gap-1 bg-white rounded-full px-4 py-1 text-sm font-medium text-gray-700 border border-gray-300"
          >
            <svg
              className="w-4 h-4"
              fill="none"
