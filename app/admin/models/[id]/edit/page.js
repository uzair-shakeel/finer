    ...
    try {
      ...
      if (data.success) {
        setFormData({
          ...
          active: data.data.active !== false, // Default to true if not specified
        });
        setOriginalBrandId(data.data.brandId || "");
      } else {
        setError(data.message || "Failed to fetch model");
        toast.error(data.message || "Failed to fetch model");
      }
    } catch (error) {
      console.error("Error fetching model:", error);
      setError("An error occurred while fetching the model");
      toast.error("An error occurred while fetching the model");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // If brand changed, redirect to the new brand's models page
        if (formData.brandId !== originalBrandId) {
          router.push(`/admin/brands/${formData.brandId}/models`);
        } else {
          router.push(`/admin/brands/${formData.brandId}/models`);
        }
      } else {
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Update the watch model information
          </p>
        </div>
        <Link
          href={`/admin/brands/${formData.brandId}/models`}
